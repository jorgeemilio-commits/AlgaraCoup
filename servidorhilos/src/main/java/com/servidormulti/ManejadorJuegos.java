...
public class ManejadorJuegos {
    ...
    public void manejarAceptar(String comando, DataOutputStream salida, UnCliente aceptador) throws IOException {
        ...
        String nombreTu = aceptador.getNombreUsuario(); 
        ...
        // Verificar si el oponente está bloqueado antes de aceptar
        if (bloqueoDB.estaBloqueado(nombreRetador, nombreTu)) {
            salida.writeUTF("No puedes aceptar el juego porque el oponente te ha bloqueado.");
            return;
        }
        if (bloqueoDB.estaBloqueado(nombreTu, nombreRetador)) {
            salida.writeUTF("No puedes aceptar el juego porque tienes bloqueado al oponente.");
            return;
        }
